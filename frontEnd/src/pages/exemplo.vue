...
<template>
  <div
    class="seat-map-viewport"
    @wheel.prevent="controlarZoom"
  ...
  >
    ...
    >
      ...
      >
        ...
        <!-- Nível 2: Assentos (renderiza apenas o setor ativo quando o zoom é alto) -->
        <div
          v-else
          class="seat-grid"
          :style="{
        ...
        >
          <button
            v-for="assento in obterAssentos(setor)"
            :key="assento.id"
            class="seat-dot"
            :class="assento.status"
            :title="`Setor ${setor.id} · Assento ${assento.numero}`"
            @click.stop="selecionarAssento(setor, assento)"
          />
        </div>
      </div>
    </div>
...
function controlarZoom(evento: WheelEvent) {
  ...
  zoom.value = Math.min(
    4,
    Math.max(0.5, zoom.value + incremento)
  )
}
...
function moverMapa(evento: MouseEvent) {
  ...
  deslocamento.x += evento.clientX - ultimaPosicao.x
  deslocamento.y += evento.clientY - ultimaPosicao.y
